...
export class WorldPropertyDatabase extends Database {
    ...
    constructor(name: string) {
        ...
        if (
            (MAX_WORLD_PROPERTY_VALUE_LENGTH + MAX_KEY_LENGTH) *
    ...
    public set(key: string, value: any): this {
        ...
        if (string.length > MAX_WORLD_PROPERTY_VALUE_LENGTH)
            throw new RangeError(
                `Value must be ${MAX_WORLD_PROPERTY_VALUE_LENGTH} (now ${string.length}) characters or less (after JSON.stringify)`
            );
